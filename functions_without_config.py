"""
Modified version of functions.py in https://github.com/Riadh-Bouarroudj/Fragile-image-watermarking-with-recovery/tree/main?tab=readme-ov-file
Particularly, the config file config-variables.py is no longer needed, as the functions work independently now
...
def self_embedding(imagex, embedding_type, wat_size_x, wat_size_y):
   img=np.copy(imagex)
   ...
   for channel in range (long): 
      ...
      down_size_x=int(image.shape[0]/wat_size_x)
      down_size_y=int(image.shape[1]/wat_size_y)
      ...
      while i+down_size_x < image.shape[0]:
         ...
         while j+down_size_y < image.shape[1]:
             ...
             # The code makes a sum of all values in LL in a block of 128x128
...
def Recovery_watermark_construction(img,long, img_size_x, img_size_y, wat_size_x, wat_size_y):
...
def embedding_DWT_watermark(cover, org_watermark, img_size_x, img_size_y, wat_size_x, wat_size_y,
                            key, key1=5, key2=10, BPP=2, bloc_size=2, 
                            self_embed=True, Auth_encryption=True, Rec_scrambling=True):
   ...
   # Load the watermarks
   if self_embed == True:
      Auth_wat = self_embedding(cover, embedding_type="DWT", wat_size_x=wat_size_x, wat_size_y=wat_size_y)
   else:
      Auth_wat = org_watermark

   Rec_wat = Recovery_watermark_construction(cover, long, img_size_x, img_size_y, wat_size_x, wat_size_y)
   Auth_arr = []
   Rec_arr1 = []
   Rec_arr2 = []
   ...
   if long == 3:
      watermarked_img = np.stack(
         [w_comp_arr[0], w_comp_arr[1], w_comp_arr[2]], axis=2
      )
   else:
...
def extraction_DWT_watermark(imagex, wat_size_x, wat_size_y, key, key1=5, key2=10, BPP=2, bloc_size=2, Auth_encryption=True, Rec_scrambling=True):
...
def recovery_process(imagex,tamper,Rec_watermark1,Rec_watermark2, img_size_x, img_size_y, wat_size_x, wat_size_y, key1=5, key2=10):   
   # Tampering is 0, 1 indicates no tampering
